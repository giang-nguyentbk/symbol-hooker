...
void perform_got_hook(int pid, const char *target_elf, const char *libfoo_elf, const char *libfake_elf) {
	void *libfoo_elf_handle = load_elf_to_memory(libfoo_elf);
	if(libfoo_elf_handle == NULL) {
		printf("Failed to load elf file %s\n", libfoo_elf);
		return;
	}
	void *libfake_elf_handle = load_elf_to_memory(libfake_elf);
	if(libfake_elf_handle == NULL) {
		printf("Failed to load elf file %s\n", libfake_elf);
		return;
	}
	...
	unsigned long libfake_elf_base_addr = get_load_module_base_address(pid, libfake_elf);
	unsigned long fake_foo_offset = get_symbol_memory_offset(libfake_elf_handle, "fake_foo");
	unsigned long fake_foo_abs_address = libfake_elf_base_addr + fake_foo_offset;
